<template>
  <UiCard class="ui-progress-chart p-4">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-bold text-white uppercase tracking-wide">Sets por ejercicio (semanal)</h3>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead>
          <tr>
            <th class="text-left px-3 py-2 text-xs font-bold uppercase tracking-wide text-gray-400">Ejercicio</th>
        ...
        <tbody>
          <tr v-for="(row, rIndex) in tableData" :key="exerciseNames[rIndex]" class="border-t border-white/10">
            <td class="px-3 py-2 font-semibold text-white">
              <span class="inline-flex items-center gap-2">
                <span class="w-3 h-3 rounded-sm" :style="{ backgroundColor: colorFor(exerciseNames[rIndex]) }"></span>
...
import { computeCompletedSetsTable } from '~~/app/utils/progressTable'
...
const table = computed(() => computeCompletedSetsTable(props.workouts, 8, props.archivedOnly ?? false))
