  ...
  <UiCard class="ui-progress-chart p-4">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
      ...
      <div class="flex flex-wrap gap-2">
        <div v-for="m in muscleNames" :key="m"
          class="flex items-center gap-2 px-2 py-1 rounded bg-white/5 border border-white/10">
          <span class="w-3 h-3 rounded-sm" :style="{ backgroundColor: colorFor(m) }"></span>
          <span class="text-xs font-bold text-gray-300 uppercase tracking-wide">{{ m }}</span>
        </div>
      </div>
    </div>
...
import { computeMuscleSetsByWeek } from '~~/app/utils/progress'
...
const maxValue = computed(() => {
  const vals: number[] = []
  weeks.value.forEach(w => {
    muscleNames.value.forEach(n => vals.push(w.totals[n] ?? 0))
  })
...
const seriesData = computed(() => {
  return muscleNames.value.map(name => {
